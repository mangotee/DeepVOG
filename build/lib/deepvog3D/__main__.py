...
The csv file must follow the format below:
    ...
    torsion  , ./fit_vid3.mp4,  ./infer_vid3.mp4,   ./model3.json,  ./output_result3.csv
...
torsion_help = 'Do torsional tracking. Call with --torsion [video_src_paht] [results_saving_path]'
...
required.add_argument("--torsion", help = torsion_help, nargs=2, type=str, metavar=("PATH","PATH"))
...
all_modes_list = [args.fit, args.infer, args.torsion, args.table, args.tui]
...
if num_modes != 1:
    ...
    if sum([x is not None for x in cli_modes_list]) >0:
        ...
        visual_save_path = args.visualize
        ...
        # Fit or Infer mode
        jobman = deepvog_jobman_CLI(gpu, flen, ori_video_shape, sensor_size, batch_size, visual_save_path)
        ...
        if args.torsion is not None:
            vid_dir, output_dir = args.torsion
            jobman.torsion(vid_dir, output_dir)
